...
#not winners
execute as @a[tag=!WinnerWinnerChickenDinner,tag=SGPlaying] run title @s subtitle [{"text": ""},{"text": "Your position: ","color": "white"},{"score":{"name":"@s","objective":"playerPosition"},"color": "green"},{"text": "/","color": "#70D470"},{"score":{"name":"$TotalPlayers","objective":"win"},"color": "#70D470"}]

...
execute if score $ZombieSpawn archeryVar matches 1 as @e[type=marker,scores={archeryZombieSpawnDelay=..0},limit=1] at @s run function sg:lobby/archery/gameplay/zombies/summon
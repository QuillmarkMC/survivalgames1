execute if score $Debug options matches 1.. run say sg:lobby/1v1/arena1/end
#determine winner
tag @a[tag=SG1v1Arena1,scores={1v1Kill=1..},limit=1] add 1v1Winner
#add to win streak
...
scoreboard players reset @a[tag=SG1v1Arena1,tag=!1v1Winner] 1v1Killstreak

#inventory
scoreboard players operation @a[tag=SG1v1Arena1,limit=2] inventoryState = $Lobby.Default inventoryState
...
schedule clear sg:lobby/1v1/arena1/timer/update
execute as @a[tag=SG1v1Arena1,limit=2] run function sg:lobby/effects/give

#remove tags
tag @a[tag=SG1v1Player1Arena1,limit=1] remove SG1v1Player1Arena1
tag @a[tag=SG1v1Player2Arena1,limit=1] remove SG1v1Player2Arena1
tag @a[tag=SG1v1Arena1,limit=2] remove SG1v1Arena1
